...
def decide_action(
    ...
    if hypertension > 0.03 or intoxication > 0.03 or alertness < 0.2:
        return 3

    # Proactive sleep to maintain alertness
    if time_since_slept > 3 or (alertness < 0.3 and time_since_slept > 2):
        return 3

    # Use coffee strategically for maintaining alertness with low hypertension
    if alertness < 0.35 and hypertension < 0.02:
        return 1
