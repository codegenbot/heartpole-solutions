...
def decide_action(alertness, hypertension, intoxication, time_since_slept, time_elapsed, work_done):
    # Prioritize sleep if low alertness or high hypertension
    if alertness < 0.5 or hypertension > 0.5 or time_since_slept > 5:
        return 3  # Sleep
    ...
    if 0.6 <= alertness < 0.9 and hypertension < 0.4 and intoxication < 0.05:
        return 1  # Drink coffee and work

    # Avoid beer unless slightly low alertness and very low health risks
    if 0.5 <= alertness < 0.6 and intoxication == 0.0 and hypertension < 0.3:
